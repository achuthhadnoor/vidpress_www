...
export const customImageCompressionCommand = (
  input: string,
  output: string,
  imageSettings: { width: number; height: number; quality: number }
): string[] => {
  const scaleOption = `scale=${imageSettings.width}:${imageSettings.height}`;
  const qualityOption = `-q:v ${imageSettings.quality}`;
  return ["-i", input, "-vf", scaleOption, qualityOption, output];
};

export const customGifCompressionCommand = (
  input: string,
  output: string,
  gifSettings: { width: number; height: number; fps: number }
): string[] => {
  const scaleOption = `scale=${gifSettings.width}:${gifSettings.height}`;
  const fpsOption = `fps=${gifSettings.fps}`;
  return ["-i", input, "-vf", `${scaleOption},${fpsOption}`, output];
};
